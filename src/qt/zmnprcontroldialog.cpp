...
#include "ui_zmnprcontroldialog.h"
...
std::list<std::string> ZMnprControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZMnprControlDialog::listMints;

ZMnprControlDialog::ZMnprControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMnprControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZMnprControlDialog::~ZMnprControlDialog()
{
    delete ui;
}

void ZMnprControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMnprControlDialog::updateList()
{
...
void ZMnprControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMnprControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZMnpr_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMnprControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZMnprControlDialog::GetSelectedMints()
{
...
void ZMnprControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
