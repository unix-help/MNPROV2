                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zMNPR ought to be enough for anybody." - Bill Gates, 2017
    ui->zMNPRpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMNPRSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzMNPR->setEnabled(false);
        ui->pushButtonMintzMNPR->setToolTip(tr("zMNPR is currently disabled due to maintenance."));

        ui->pushButtonSpendzMNPR->setEnabled(false);
        ui->pushButtonSpendzMNPR->setToolTip(tr("zMNPR is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMNPRpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMNPR_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMNPR is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMNPR...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMNPR in ") +
...
void PrivacyDialog::on_pushButtonSpendzMNPR_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMNPR is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMNPR
        sendzMNPR();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMNPR
    sendzMNPR();
}

void PrivacyDialog::on_pushButtonZMnprControl_clicked()
{
    ZMnprControlDialog* zMnprControl = new ZMnprControlDialog(this);
    zMnprControl->setModel(walletModel);
    zMnprControl->exec();
}

void PrivacyDialog::setZMnprControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMnprSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMNPR()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Mnpro Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMNPRpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMNPR is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MNPR </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMNPR</b>";
    ...
    // use mints from zMnpr selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZMnprControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZMnprControlDialog::GetSelectedMints();
    }
    ...
    // Clear zmnpr selector in case it was used
    ZMnprControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMnpr Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMNPR </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMNPR "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMNPR "));
    ui->labelzMNPRAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMNPRSyncStatus->setVisible(fShow);
}
