...
#if defined(NDEBUG)
#error "Mnpro cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zMnpr spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zMnpr spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zMnpr spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
	...
	if(nHeight ==0) {
        ...
        nSubsidy = 10 * COIN;
	}else if(nHeight > 20000 && nHeight <= 90000) {
        nSubsidy = 30 * COIN;
	} else if (nHeight > 90000 && nHeight <= 150000) {
		nSubsidy = 40 * COIN;
	} else if (nHeight > 150000 && nHeight <= 300000) {
		nSubsidy = 50 * COIN;
	} else if (nHeight > 300000 && nHeight <= 350000) {
		nSubsidy = 100 * COIN;
	} else if (nHeight > 350000 && nHeight <= 390000) {
		nSubsidy = 75 * COIN;
	} else if (nHeight > 390000 && nHeight <= 410000) {
		nSubsidy = 50 * COIN;
	} else if (nHeight > 410000 && nHeight <= 411000) { 
		nSubsidy = 500 * COIN;
	} else if (nHeight > 411000 && nHeight <= 540000) { 
		nSubsidy = 60 * COIN;
	} else if (nHeight > 540000 && nHeight <= 580000) {
		nSubsidy = 25 * COIN;
	} else if (nHeight > 580000 && nHeight <= 625000) {
		nSubsidy = 10 * COIN;
...
if(nHeight > 0 && nHeight <= 20000) {
        ...
        ret = blockValue / 100 * 85;
	} else if (nHeight > 150000 && nHeight <= 152500) {
        ret = blockValue / 100 * 85;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Mnpro
...
{
    RenameThread("mnpro-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZMNPRMinted()
{
...
void RecalculateZMNPRSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zMNPR supply
...
bool RecalculateMNPRSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zMnpr with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zMnprSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MNPR to zMNPR
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zMnpr spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zMnpr serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zMnpr mints and %d zMnpr spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Mnpro: We use certain sporks during IBD, so check to see if they are
