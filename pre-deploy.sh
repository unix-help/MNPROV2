...
if [[ $HOST = "i686-w64-mingw32" ]]; then
  ARCHIVE_NAME="mnpr-windows-32bit.zip"
elif [[ $HOST = "x86_64-w64-mingw32" ]]; then
    ARCHIVE_NAME="mnpr-windows-64bit.zip"
    ...
    ARCHIVE_NAME="mnpr-linux-x64.tar.gz"
    ARCHIVE_CMD="tar -czf"
...
if [[ $HOST = "x86_64-apple-darwin11" ]]; then
    find $TRAVIS_BUILD_DIR -type f | grep -i Mnpro-Core.dmg$ | xargs -i cp {} $OUTDIR/bin
else
    cp $TRAVIS_BUILD_DIR/src/qt/mnpro-qt $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/qt/mnpro-qt.exe $OUTDIR/bin/ || echo "no QT Wallet"
    cp $TRAVIS_BUILD_DIR/src/mnprod $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/mnprod.exe $OUTDIR/bin/
    cp $TRAVIS_BUILD_DIR/src/mnpro-cli $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/mnpro-cli.exe $OUTDIR/bin/
